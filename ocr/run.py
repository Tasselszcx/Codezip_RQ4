    ...
    if OCR_CONCURRENCY <= 1:
        for i, (code_id, ratio, page_paths) in enumerate(cases, start=1):
            ...
            rec = {
                ...
                # 保留 image_path 字段用于兼容旧的下游逻辑/工具（取第一页）
...
def _compute_codediff_metrics_no_blank(reference: str, hypothesis: str) -> dict:
    """对 reference/hypothesis 去空行后做行级 difflib 对比，输出 diff 指标。"""
...
def run_module_4_judge(
    ...
    for idx, ((code_id, ratio), pages) in enumerate(grouped.items(), 1):
        ...
        detail_rec = {
            ...
            "codediff_no_blank": codediff_no_blank,
        ...
        if ratio in stats_by_ratio:
            ...
            stats_by_ratio[ratio]["codediff_line_sim_sum"] += codediff_no_blank["line_similarity"]
            stats_by_ratio[ratio]["codediff_change_rate_sum"] += codediff_no_blank["change_rate"]
            stats_by_ratio[ratio]["codediff_added_sum"] += codediff_no_blank["added_lines"]
            stats_by_ratio[ratio]["codediff_removed_sum"] += codediff_no_blank["removed_lines"]
            stats_by_ratio[ratio]["codediff_replaced_sum"] += codediff_no_blank["replaced_lines"]
            stats_by_ratio[ratio]["codediff_hunks_sum"] += codediff_no_blank["diff_hunks"]
...
def run_full_process():
    ...
    to_remove = [glm_judge_detail, glm_judge_summary]
    # 只有在要重新跑 OCR 时才删除 ocr.jsonl；只跑 Module 4 时保留现有 OCR 结果。
    if RUN_MODULE_3:
        to_remove.insert(0, glm_ocr_jsonl)
